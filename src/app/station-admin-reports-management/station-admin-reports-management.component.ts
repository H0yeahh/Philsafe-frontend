...
@Component({
  selector: 'app-station-admin-reports-management',
  templateUrl: './station-admin-reports-management.component.html',
  styleUrl: './station-admin-reports-management.component.css'
})
export class StationAdminReportsManagementComponent {
      ...
      this.caseQueueService.getReportsPage(stationId, pageNumber, this.pageSize).subscribe(
        (response) => {
          ...
          if (reports.length < this.pageSize) {
            ...
            this.spamReports = this.reports.filter((report: any) => report.is_spam === true);
  ...
  reinstateReport(reportId: number){

    this.dialogService.openConfirmationDialog(`Reinstating Report ${reportId} will be moved for further investigation. Would you like to proceed?`).then(
      (confirm) => {
        if(confirm){
          this.dialogService.openLoadingDialog();
          this.caseQueueService.reinstateReport(reportId).subscribe(
            (res) => {
              // alert(`Report ${reportId} has been successfully deleted.`);
      
              setTimeout(() => {
                this.dialogService.closeLoadingDialog();
                this.dialogService.openUpdateStatusDialog('Success', `Report ${reportId} has been successfully reinstated.`);
            ...
            (err) => {
              console.error('Failed to reinstate reports', err)
              this.dialogService.closeAllDialogs();
              this.dialogService.openUpdateStatusDialog('Failed', `Failed to reinstate Report ${reportId}. Contact you Regional Administrator.`);
              this.reports = this.reports.filter(report => report.report_id !== reportId);
            }
          );
        } else {
          this.dialogService.openUpdateStatusDialog('Cancelled', 'Reinstating cancelled');
        }
      }
    )
  ...
  deleteReport(reportId: number) {
    ...
    this.dialogService.openConfirmationDialog(`Are you sure you want to permanently delete Report ${reportId}?`).then(
      (confirmed) => {
        if(confirmed){
          this.dialogService.openLoadingDialog();
    this.caseQueueService.spamReport(reportId).subscribe(
      ...
      (err) => {
        ...
        this.dialogService.closeAllDialogs();
        this.dialogService.openUpdateStatusDialog('Failed', `Failed to delete Report ${reportId}. Contact you Regional Administrator.`);
        this.reports = this.reports.filter(report => report.report_id !== reportId);
      }
    );
          ...
          this.dialogService.openUpdateStatusDialog('Cancelled', 'Deletion cancelled');
